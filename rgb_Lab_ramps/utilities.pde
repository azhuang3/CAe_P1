...
class COLOR_RAMP {
  ...
  color atLab(int i , int n) {return atLab(float(i)/n); }
  color atLab(float s) {return LABramp( redC(c0), greenC(c0), blueC(c0), redC(c1), greenC(c1), blueC(c1), s);}
  
  color[] colorLabMap(int n) {
